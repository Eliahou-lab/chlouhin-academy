    ...
    <div className="sticky top-20 z-30 rounded-xl border border-border bg-surface/95 p-3 shadow-xl backdrop-blur">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button className="rounded-md border border-border px-3 py-2 text-sm disabled:opacity-40" disabled={!focusEnabled || index === 0} onClick={() => setIndex((value) => Math.max(0, value - 1))}>
            Précédent
          </button>
          <button className="rounded-md bg-primary px-3 py-2 text-sm font-semibold text-white disabled:opacity-40" disabled={!focusEnabled || index >= safeItems.length - 1} onClick={() => setIndex((value) => Math.min(safeItems.length - 1, value + 1))}>
            Suivant
          </button>
        </div>
      </div>
